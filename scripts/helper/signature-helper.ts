...
const signTypedData = async (
  ...
  const adjustedSigner = new Wallet(signer);
  return { ...adjustedSigner._signingKey().signDigest(digest) };
};

export const computeDomainSeparator = (verifyingContract: string, chainId: string): string => {
  const domain: TypedDataDomain = {
    name: "LooksRareExchange",
    version: "1",
    chainId: chainId,
...
export const signMakerOrder = (
  ...
  return signTypedData(signer, types, values, verifyingContract, chainId);
};
